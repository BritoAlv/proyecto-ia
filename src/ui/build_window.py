...
class BuildWindow(QMainWindow):
    def __init__(self, height: int = 40, width: int = 40):
        ...
        self._control_widgets[_Widgets.remove_place_button] = QPushButton(
            "Remove Place"
        )
        ...
        self._control_widgets[_Widgets.place_name_input].setPlaceholderText(
            "Place's name"
        )
        self._control_widgets[_Widgets.place_description_input] = QTextEdit()
        self._control_widgets[_Widgets.place_description_input].setPlaceholderText(
            "Place's description"
        )
        ...
        self._control_widgets[_Widgets.back_home_button].clicked.connect(
            self._handle_back_home
        )
        self._control_widgets[_Widgets.add_road_button].clicked.connect(
            self._handle_add_road
        )
        self._control_widgets[_Widgets.remove_road_button].clicked.connect(
            self._handle_remove_road
        )
        self._control_widgets[_Widgets.stop_button].clicked.connect(self._handle_stop)
        self._control_widgets[_Widgets.zoom_in_button].clicked.connect(
            self._handle_zoom_in
        )
        self._control_widgets[_Widgets.zoom_out_button].clicked.connect(
        ...
        self._control_widgets[_Widgets.enter_name_button].clicked.connect(
            self._handle_enter_name
        )
        self._control_widgets[_Widgets.add_place_button].clicked.connect(
            self._handle_add_place
        )
        self._control_widgets[_Widgets.remove_place_button].clicked.connect(
            self._handle_remove_place
        )
        self._control_widgets[_Widgets.enter_place_button].clicked.connect(
            self._handle_enter_place
        )
    ...
    def _handle_add_road(self):
        self._add_road = True
        self._show_widgets(
            lambda widget_id: widget_id
            in [_Widgets.stop_button, _Widgets.zoom_in_button, _Widgets.zoom_out_button]
        )
    ...
    def _handle_name(self):
        self._show_widgets(
            lambda widget_id: widget_id
            in [_Widgets.enter_name_button, _Widgets.name_input]
        )
        ...
        for i in range(self._grid_height):
            ...
            for j in range(self._grid_width):
                if (
                    i == 0
                    or j == 0
                    or i == self._grid_height - 1
                    or j == self._grid_width - 1
                ):
                    block_matrix[i].append(
                        SidewalkBlock((i, j), i == 0 or i == self._grid_height - 1)
                    )
                    continue
                ...
                if direction == Directions.INTERSECTION:
                    ...
                    for p, q in offsets:
                        ...
                        if isinstance(neighbor_block, SemaphoreBlock):
                            block_matrix[i][j] = SemaphoreBlock(
                                (i, j), neighbor_block.representative
                            )
                            break
                    ...
                    for p, q in two_offsets:
                        ...
                        if not valid_coordinates(
                            x, y, self._grid_height, self._grid_width
                        ):
                            continue

                        if self._matrix[x][y] in [
                            Directions.NORTH,
                            Directions.SOUTH,
                            Directions.EAST,
                            Directions.WEST,
                    ...
                    meta_data = self._get_place_meta_data(name, description)
                    block_matrix[i].append(
                        PlaceBlock((i, j), name, description, representative, meta_data)
                    )
                    ...
                    for p, q in offsets:
                        if valid_coordinates(
                            i + p, j + q, self._grid_height, self._grid_width
                        ) and self._matrix[i + p][j + q] not in [
                            Directions.EMPTY,
                            Directions.PLACE,
                        ]:
    ...
    def _handle_add_place(self):
        ...
        self._show_widgets(
            lambda widget_id: widget_id
            in [
                _Widgets.place_name_input,
                _Widgets.place_description_input,
                _Widgets.zoom_in_button,
                _Widgets.zoom_out_button,
                _Widgets.enter_place_button,
            ]
        )
    ...
    def _handle_enter_place(self):
        ...
        place_description = self._control_widgets[
            _Widgets.place_description_input
        ].toPlainText()
    ...
    def _handle_tile_clicked(self, coordinates: tuple[int, int]):
        ...
        if self._add_road and self._map_border_predicate(i, j):
            ...
            if vertical:
                ...
                self._matrix[0][j] = self._matrix[self._grid_height - 1][j] = (
                    Directions.EOR
                )
                ...
                self._matrix[i][0] = self._matrix[i][self._grid_width - 1] = (
                    Directions.EOR
                )
            ...
            for k in range(length):
                ...
                if self._matrix[p][q] != Directions.EMPTY:
                    ...
                    self._tiles[p][q].set_color(
                        DIRECTION_COLOR[Directions.INTERSECTION]
                    )
                else:
                    self._tiles[p][q].set_color(color)
                    self._matrix[p][q] = direction
        elif self._remove_road and direction not in [
            Directions.EMPTY,
            ...
            if vertical:
                ...
                self._tiles[self._grid_height - 1][j].set_color(
                    DIRECTION_COLOR[Directions.EMPTY]
                )
            else:
                # Enable start-column
                self._matrix[i][0] = self._matrix[i][self._grid_width - 1] = (
                ...
                self._tiles[i][self._grid_width - 1].set_color(
                    DIRECTION_COLOR[Directions.EMPTY]
                )
            ...
            for k in range(length):
                ...
                if self._matrix[p][q] != Directions.INTERSECTION:
                    ...
                    for offset in [(0, 1), (0, -1)] if vertical else [(1, 0), (-1, 0)]:
                        ...
                        if self._matrix[p_prime][q_prime] not in [
                            Directions.EMPTY,
                            Directions.INTERSECTION,
                        ]:
    ...
    def _home_widgets_predicate(self, widget_id):
        return widget_id not in [
            _Widgets.stop_button,
            _Widgets.enter_name_button,
            _Widgets.enter_place_button,
            _Widgets.name_input,
            _Widgets.place_name_input,
            _Widgets.place_description_input,
    ...
    def _map_border_predicate(self, i: int, j: int):
        first_border = (
            i == 0 or j == 0 or i == self._grid_height - 1 or j == self._grid_width - 1
        )

        no_corner = (
            (i, j) != (0, 0)
            and (i, j) != (self._grid_height - 1, self._grid_width - 1)
            and (i, j) != (0, self._grid_width - 1)
            and (i, j) != (self._grid_height - 1, 0)
        )
    ...
    def _available_place_predicate(self, i: int, j: int):
        ...
        for p, q in DIRECTION_OFFSETS.values():
            ...
            if self._matrix[i][j] == Directions.EMPTY and self._matrix[x][y] in [
                Directions.NORTH,
                Directions.SOUTH,
                Directions.EAST,
                Directions.WEST,
    ...
    def _get_place_meta_data(self, name: str, description: str) -> dict:
        nlp_input = json.dumps({"name": name, "description": description})
